	...
	if (hComm != NULL)
	{
		...
		if (osRead.hEvent != NULL)
		{
			...
			if (ReadFile(hComm, &Buffer, dwToRead, &dwRead, &osRead))
			{
				...
				Buffer[dwRead] = '\0';
				memcpy(&buffer[0], &Buffer[0], dwRead*sizeof(unsigned char));
            }
            else
            {
...
//---------------------------------------------------------------------------
std::vector <AnsiString> SerialPort::CheckSerialPort()
{
    HANDLE hCommCheck;
    AnsiString asCommPort;
    AnsiString asPort;
    ...
    for (int a = 1; a < 30; a++)
    {
        ...
        // Abertura da porta serial selecionada.
        hCommCheck = CreateFileA(asCommPort.c_str(),           // ponteiro para a porta selecionada.
                                 GENERIC_READ | GENERIC_WRITE, // Modo de acesso (escrita ou leitura).
                                 0,                            // Não utilizado.
                                 0,                            // Não utilizado.
                                 CREATE_ALWAYS,                // Como a porta será criada.
